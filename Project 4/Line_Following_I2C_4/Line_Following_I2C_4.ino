...
int motor_speed[4];
const int motor_speed_init[4] = { 60,60,60,60 };
const int motor_position[4] = { 1,1,-1,-1 };//1 for left,-1 for right;

unsigned int sensorData[10] = { 0 };     // Array for formatted data from module
...
float get_position(void)
{
    ...
    if (sensorData[1] > 60)return -100;
    if (sensorData[8] > 60)return 169;
...
void get_feedback_data(void)
{
    ...
    LCD_PRINT.left_speed = (int)(speed_dir_1 * (int)(tmp * 0.55));
...
/**
* @brief    :   check short cut
* @return   :   if the short cut have been find out
*   @retval :   0 : no short cut
*/
inline bool check_shortcut(void)
{
    int road_num[2];
    int num = 0;
    for (int i = 1;i <= 8;i++)
    {
        if (larger_determine(sensorData[i - 1], sensorData[i], sensorData[i + 1]))
        {
            road_num[num] = i;
            num++;
            if (num >= 3)return 0;
    ...
    if (num <= 1)return 0;
    if (sensorData[road_num[0]] > sensorData[road_num[1]])
    {
        sensorData[road_num[0]] = sensorData[road_num[0] - 1] + sensorData[road_num[0] + 1];
        return 1;
    }
    else
    {
        sensorData[road_num[1]] = sensorData[road_num[1] - 1] + sensorData[road_num[1] + 1];
        return -1;
    }
}

#define LINE_READER_DELTA     20



/**
* @brief    : if find a short maxi
* @param[in]: a : the velue before
* @param[in]: a : the velue before
* @param[in]: a : the velue before
* @return   :if the line if found
*   @retval :   0: not a road
*   @retval :   1: find a road
*/
inline bool larger_determine(int a,int b,int c)
{
    if (((b - a) > LINE_READER_DELTA) && ((b - c) > LINE_READER_DELTA))
    {
        return true;
    }
}