...
#define MIN_GRADE   5
#define GRADE_INCREASE_DISTANCE   ONE_METER/100*(38.516+51.825)//<< the number increase of the grade

#define TASK_DELAY     delay(5)
...
int forward_speed[4] = { 70,70,70,70 };
int backward_speed[4] = { -70,-70,-70,-70 };
int left_speed[4] = { -70,-70,70,70 };
int right_speed[4] = { 70,70,-70,-70 };
int left_round_speed[4] = { 50,50,70,70 };
...
void loop() {
    go_straight(1, ONE_METER * 2.5);
    TASK_DELAY;
    turn(1, 45);
    TASK_DELAY;
    go_straight(1, (long unsigned)((float)ONE_METER * 1.41421));
    TASK_DELAY;
    turn(1, 45);
    TASK_DELAY;
    go_straight(0, ONE_METER * 0.5);
    TASK_DELAY;
    turn(1, 63);
    TASK_DELAY;
    go_straight(1, (long unsigned)((float)ONE_METER * 1.11803));
    TASK_DELAY;
    turn(1, 27);
    TASK_DELAY;
    go_straight(1, ONE_METER * 3);
    TASK_DELAY;
    turn(1, 45);
    TASK_DELAY;
    go_straight(1, (long unsigned)((float)ONE_METER * 0.70710));
    TASK_DELAY;
    turn(1, 45);
    TASK_DELAY;
    turn_round(1, 90);
    TASK_DELAY;
    go_straight(1, ONE_METER * 0.5);
    while (1)
        continue;
}
...
void get_feedback_data(void)
{
    ...
    info.sta = ((tmp < 10) ? 0 : 1);
    info.inclination = mpu_real_data[pitch_axis];
}
...
{
    if (abs(mpu_real_data[yaw_axis] - angle_set_begin) < 5)
    {
        return 0;
    }
...
{
    angle_target = target;
    angle_set_begin = (mpu_real_data[yaw_axis]);
}
