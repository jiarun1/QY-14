...
void loop() {
    readEncoder1();
    go_straight(0, motor_speed_set, 1000);
    block_straight();
...
void vehicle_control(void)
{
    ...
    interrupt_happed = !interrupt_happed;
...
void block_straight(void)
{
    while (1)
    {
        mpu_get_data();
        readEncoder1();
        info.distance += abs(encoder1Value - distance_set_begin);
        if (last_interrupt != interrupt_happed)
        {
            last_interrupt == interrupt_happed;
            rf24.write((uint8_t*)&info, sizeof(info));
        }
        if (!check_distance())
        {
            break;
...
bool check_distance(void)//1 if can keep running
{
...
void go_straight(bool direction_1, int vehicle_speed[4], long unsigned int distance)
{
    //noInterrupts();
